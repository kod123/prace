  ...
  <div id="obal">
    ...
    <form method="POST" action="registrace.php" enctype="multipart/form-data">
      ...
      <fieldset>
        ...
        <a href="velikonoce.php" id="prihlaseni">Přihlásit se</a>
      </fieldset>
...
class registrace
{
  ...
  // Metoda pro zavolání připojovací třídy k databázi + vypsání přípádné chyby při nenavázání spojení  
   ...
   {
     $shodaUzivatelMysql = $mojeMysql->query("SELECT COUNT(*) AS cisloUzivatel FROM uzivatele WHERE uzivatel='$this->uzivatelV'");
     $shodaEmailMysql = $mojeMysql->query("SELECT COUNT(*) AS cisloEmail FROM uzivatele WHERE email='$this->emailV'");
  ...
  elseif(empty($_POST["jmeno"]) && isset($_POST["prijmeni"]) && isset($_POST["uzivatel"]) && isset($_POST["email"]) && isset($_POST["heslo"]) && isset($_POST["hesloPotvrzeni"]) && isset($_POST["tlacitko"]))
  {
    echo ("Musíte vyplnit jméno!!!");
     ...
     $mojeMysql->query("INSERT INTO uzivatele (jmeno, prijmeni, uzivatel, heslo, email, datum, kod, aktivace) VALUES ('$jmenoVstupVelke','$prijmeniVstupVelke', '$this->uzivatelV', '$this->hesloV', '$this->emailV', NOW(), '$this->kod', '0')"); 
